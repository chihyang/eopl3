...
;; refarray : Array x Int -> Ref
(define ref-of-array-element
  (lambda (arr n)
    (cases array arr
           (an-array
            (val size)
...
(eqv?
 (run "let a = newarray(4,-99)
       ...
       in letrec arrayinit(x, i) =
                   if zero?(i) then 98
                   else begin
                          arrayset(x, -(i, 1), i);
                          (arrayinit x -(i, 1))
                        end
          in begin (arrayinit a arraylength(a));
...
(eqv?
 (run "let a = newarray(4,-99)
       ...
       in letrec arrayinit(x, i) =
          ...
          in ((swap arrayref(a, arrayref(a, 0))) arrayref(a, 1))")
 27)
