...
(define set-rhs-cont
  (lambda (ref cont)
    (cons (set-rhs-frame ref) cont)))
(define begin-exp-cont
  (lambda (exp env cont)
    (cons (begin-exp-frame exp env) cont)))
(define spawn-cont
  (lambda (cont)
    (cons (spawn-frame) cont)))
(define end-subthread-cont
  (lambda ()
    (list 'end-subthread-cont)))
(define end-mainthread-cont
  (lambda ()
    (list 'end-mainthread-cont)))
(define wait-cont
  (lambda (cont)
...
(define print-cont
  (lambda (cont)
    (cons (print-frame) cont)))
(define yield-cont
  (lambda (cont)
    (cons (yield-frame) cont)))
    ...
    (if (time-expired?)
        ...
        (begin
          ...
          (let ((cur-frame (car cont))
                (next-cont (cdr cont)))
            (cond [(eq? cur-frame 'end-mainthread-cont)
                   (when (debug-mode?)
                     (eopl:printf "Main thread exit.~%"))
                   (set-final-answer! val)
                   (run-next-thread)]
                  [(eq? cur-frame 'end-subthread-cont)
                   (when (debug-mode?)
                  ...
                  [else
                   (cases frame cur-frame
                          ...
                          (set-rhs-frame
                           ...
                           (begin
                             (setref! ref val)
                             (apply-cont next-cont (num-val 27))))
                          (begin-exp-frame
                           (exp1 saved-env)
                           (value-of/k exp1 saved-env next-cont))
                          (spawn-frame
                           ()
                           (let ((proc1 (expval->proc val)))
                             (place-on-ready-queue!
                              (proc-thread
                               ...
                               (list (num-val 28))
                               (end-subthread-cont)))
                             (apply-cont next-cont (num-val 73))))
                          (wait-frame
                           ()
                           (wait-for-mutex
                            (expval->mutex val)
                          ...
                          (print-frame
                           ...
                           (begin
                             (print-expval val)
                             (apply-cont next-cont (num-val 33))))
                          (yield-frame
                           ()
                           (begin
                             (when (debug-mode?)
                             ...
                             (if (time-expired?)
                                 (place-on-ready-queue!
                                  (cont-thread the-time-remaining next-cont val))
                                 (place-on-ready-queue!
                                  (cont-thread the-max-time-slice next-cont val)))
                             (run-next-thread))))]))))))
