...
;; apply-procedure/k : Proc x Listof(ExpVal) x Cont x Cont -> Bounce
(define apply-procedure/k
  (lambda (proc1 vals cont enp-cont)
    (lambda ()
      (cases proc proc1
             (procedure
              (vars body saved-env)
              (value-of/k body (extend-env* vars (map newref vals) saved-env) cont enp-cont))))))
...
(define-datatype continuation continuation?
  ...
  (raise1-cont
   (saved-enp-cont continuation?)
   (saved-cont continuation?)))
;; apply-cont : Cont x Cont x ExpVal -> Bounce
(define apply-cont
  (lambda (cont enp-cont val)
    (cases continuation cont
           ...
           (zero1-cont
            (cont)
            (apply-cont cont enp-cont (bool-val (zero? (expval->num val)))))
           (let-exp-cont
            (var body env cont)
            (value-of/k body (extend-env var val env) cont enp-cont))
           (if-test-cont
            (exp2 exp3 env cont)
            (if (expval->bool val)
           ...
           (diff1-cont
            (exp2 env cont)
            (value-of/k exp2 env (diff2-cont val cont) enp-cont))
           (diff2-cont
            (val1 cont)
           ...
           (rator-cont
            (exps env cont)
            (if (null? exps)
                (apply-procedure/k (expval->proc val) '() cont enp-cont)
                (value-of/k (car exps) env (rand-cont val '() (cdr exps) env cont) enp-cont)))
           (rand-cont
            (rator saved-vals cont-exps env cont)
            (if (null? cont-exps)
                (let ((proc1 (expval->proc rator)))
                  (apply-procedure/k proc1 (reverse (cons val saved-vals)) cont enp-cont))
           ...
           (cons1-cont
            (exp2 env cont)
            (value-of/k exp2 env (cons2-cont val cont) enp-cont))
           (cons2-cont
            (val1 cont)
            (apply-cont cont enp-cont (pair-val val1 val)))
           (car-cont
            (cont)
            (cases exp-val val
                   (pair-val
                    (first rest)
                    (apply-cont cont enp-cont first))
           ...
           (null1-cont
            ...
            (cases exp-val val
                   (null-val
                    ()
                    (apply-cont cont enp-cont (bool-val #t)))
                   (else (apply-cont cont enp-cont (bool-val #f)))))
           (list1-cont
            (exps env cont)
            (value-of/k exps env (list2-cont val cont) enp-cont))
           (list2-cont
            (first-val cont)
            (apply-cont cont enp-cont (pair-val first-val val)))
           (set-rhs-cont
            (ref cont)
            (begin
              (setref! ref val)
              (apply-cont cont enp-cont (num-val 27))))
           (begin-exp-cont
            (exp env cont)
            (value-of/k exp env cont enp-cont))
           (try-cont
            (var cont-var handler-exp saved-env saved-cont saved-enp-cont)
            (apply-cont saved-cont enp-cont val))
           (resume1-cont
            (exp2 saved-env saved-cont saved-enp-cont)
            (value-of/k exp2 saved-env (resume2-cont val) enp-cont))
           (resume2-cont
            (saved-val)
            (apply-cont (expval->cont saved-val) enp-cont val))
           (raise1-cont
            (saved-enp-cont saved-cont)
            (apply-handler saved-enp-cont (cont-val saved-cont) val)))))
;;; apply-handler : Cont x Cont x ExpVal -> FinalANswer
(define apply-handler
  (lambda (cont cont-val val)
    (cases continuation cont
           (try-cont
            (raised-var cont-var handler-exp saved-env saved-cont saved-enp-cont)
            (value-of/k handler-exp
                        (extend-env* (list raised-var cont-var) (map newref (list val cont-val)) saved-env)
                        saved-cont
                        saved-enp-cont))
           (else
...
;;;                try-exp (exp1 var cont-var handler-exp)
;;; Expression ::= raise Expression
;;;                raise-exp (exp)
;;; Expression ::= resume exp with exp
...
(define let-grammar
  '((program (expression) a-program)
    ...
    (expression ("raise" expression)
                raise-exp)
    (expression ("resume" expression "with" expression)
                resume-exp)))

;;; ---------------------- Evaluate expression ----------------------
;; value-of/k : Exp x Env x Cont x Cont -> Bounce
(define value-of/k
  (lambda (exp env cont enp-cont)
    (cases expression exp
           (const-exp
            (num)
            (apply-cont cont enp-cont (num-val num)))
           (var-exp
            (var)
            (apply-cont cont enp-cont (deref (apply-env env var))))
           (emptylist-exp
            ()
            (apply-cont cont enp-cont (null-val)))
           (cons-exp
            (exp1 exp2)
            (value-of/k exp1 env (cons1-cont exp2 env cont) enp-cont))
           (car-exp
            (exp1)
            (value-of/k exp1 env (car-cont cont) enp-cont))
           (cdr-exp
            (exp1)
            (value-of/k exp1 env (cdr-cont cont) enp-cont))
           (null?-exp
            (exp1)
            (value-of/k exp1 env (null1-cont cont) enp-cont))
           (list-exp
            (exps)
            (if (null? exps)
                (apply-cont cont enp-cont (null-val))
                (value-of/k (car exps) env (list1-cont (list-exp (cdr exps)) env cont) enp-cont)))
           (diff-exp
            (exp1 exp2)
           ...
           (let-exp
            (vars exps body)
            (if (null? vars)
                (value-of/k body env cont enp-cont)
                (value-of/k (car exps) env (let-cont '() '()  vars (cdr exps) body env cont) enp-cont)))
           (letrec-exp
            (p-names p-vars p-bodies letrec-body)
            (value-of/k letrec-body (extend-env-rec p-names p-vars p-bodies env) cont enp-cont))
           (proc-exp
            (vars body)
            (apply-cont cont enp-cont (proc-val (procedure vars body env))))
           (call-exp
            (rator rand)
            (value-of/k rator env (rator-cont rand env cont) enp-cont))
           (begin-exp
            (exp1 exps)
            (if (null? exps)
                (value-of/k exp1 env cont enp-cont)
                (value-of/k exp1 env (begin-exp-cont (begin-exp (car exps) (cdr exps)) env cont) enp-cont)))
           (assign-exp
            (var exp1)
            (let ((ref (apply-env env var)))
              (value-of/k exp1 env (set-rhs-cont ref cont) enp-cont)))
           (try-exp
            (exp1 raised-var cont-var handler-exp)
            (value-of/k exp1 env cont (try-cont raised-var cont-var handler-exp env cont enp-cont)))
           (resume-exp
            (exp1 exp2)
            (value-of/k exp1 env (resume1-cont exp2 env cont enp-cont) enp-cont))
           (raise-exp
            (exp1)
            (cases continuation enp-cont
                   (try-cont
                    (saved-var saved-cont-var handler-exp saved-env saved-cont saved-enp-cont)
                    (value-of/k
                     exp1 env (raise1-cont enp-cont cont) enp-cont))
                   (else
                    (begin
                      (report-uncaught-exception)
                      #f)))))))
...
(define value-of-program
  (lambda (prog)
    ...
    (cases program prog
           (a-program
            (exp)
            (let ((val (trampoline (value-of/k exp (empty-env) (end-cont) (end-cont)))))
              (if (exp-val? val)
                  (cases exp-val val
                         (num-val
                          (num)
...
(check-eqv?
 (run
  "letrec index (n, lst)
   ...
   in try (index 5 list(2, 3))
      catch (x, cont) resume cont with -1")
 1)
...
(check-eqv?
 (run
  "letrec index (n, lst)
   ...
   in try (index 5 list(2, 3))
      catch (x, y) resume y with raise x")
 #f)
