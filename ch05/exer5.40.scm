...
(define apply-cont
  (lambda (cont enp-cont val)
    (cases continuation cont
           ...
           (try-cont
            (var handler-exp saved-env saved-cont saved-enp-cont)
...
;;; Expression ::= recoverable-try Expression catch (Identifier) Expression
;;;                resume-exp (exp1 var handler-exp)
  ...
  (lambda (exp env cont enp-cont)
    (cases expression exp
           ...
           (try-exp
            ...
            (value-of/k exp1 env cont (try-cont var handler-exp env cont enp-cont)))
           (resume-exp
            (exp1 var handler-exp)
            (value-of/k exp1 env cont (resume-cont var handler-exp env cont enp-cont)))
           (raise-exp
            (exp1)
            (cases continuation enp-cont
                   (try-cont
                    (saved-var handler-exp saved-env saved-cont saved-enp-cont)
                    (value-of/k
                     exp1 env (raise1-cont enp-cont) enp-cont))
                   (resume-cont
                    (saved-var handler-exp saved-env saved-cont saved-enp-cont)
                    (value-of/k
                     exp1 env
                     (raise1-cont
                      (resume-cont saved-var handler-exp saved-env cont saved-enp-cont))
                     enp-cont))
                   (else
                    (begin
                      (report-uncaught-exception)
...
(check-eqv?
 (run
  "letrec index (n, lst)
   ...
   in recoverable-try (index 5 list(2, 3))
      catch (x) -1")
 1)
...
(check-eqv?
 (run
  "letrec index (n, lst)
   ...
   in begin
        recoverable-try (index 5 list(2, 3))
        catch (x)
            recoverable-try
              raise -(x, raise x)
            catch (x)
