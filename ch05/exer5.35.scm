...
(define apply-cont
  (lambda (cont val)
    (cases continuation cont
           ...
           (let-exp-cont
            (var body env cont enp-cont)
            (value-of/k body (extend-env var val env) cont))
           (if-test-cont
            (exp2 exp3 env cont enp-cont)
            (if (expval->bool val)
                (value-of/k exp2 env cont)
                (value-of/k exp3 env cont)))
           (diff1-cont
            (exp2 env cont enp-cont)
            (value-of/k exp2 env (diff2-cont val cont (get-enp-cont cont))))
           (diff2-cont
            (val1 cont enp-cont)
           ...
           (rator-cont
            (exps env cont enp-cont)
            (if (null? exps)
                (apply-procedure/k (expval->proc val) '() cont)
                (value-of/k (car exps) env
                            (rand-cont val '() (cdr exps) env cont (get-enp-cont cont)))))
           (rand-cont
            (rator saved-vals cont-exps env cont enp-cont)
            (if (null? cont-exps)
                ...
                (value-of/k (car cont-exps)
                            ...
                            (rand-cont rator (cons val saved-vals) (cdr cont-exps) env cont
                                       (get-enp-cont cont)))))
           (let-cont
            (saved-vars saved-vals cont-vars cont-exps body env cont enp-cont)
            (if (null? cont-exps)
           ...
           (cons1-cont
            (exp2 env cont enp-cont)
            (value-of/k exp2 env (cons2-cont val cont (get-enp-cont cont))))
           (cons2-cont
            (val1 cont enp-cont)
           ...
           (list1-cont
            (exps env cont enp-cont)
            (value-of/k exps env (list2-cont val cont (get-enp-cont cont))))
           (list2-cont
            (first-val cont enp-cont)
           ...
           (begin-exp-cont
            (exp env cont enp-cont)
            (value-of/k exp env cont))
           (try-cont
            (var handler-exp env cont)
            (apply-cont cont val))
           (raise1-cont
            (cont)
...
(define apply-handler
  (lambda (cont val)
    (cases continuation cont
           (try-cont
            (var handler-exp env cont)
            (value-of/k handler-exp (extend-env var (newref val) env) cont))
           (else
            (begin
...
(define value-of/k
  (lambda (exp env cont)
    (cases expression exp
           ...
           (cons-exp
            (exp1 exp2)
            (value-of/k exp1 env (cons1-cont exp2 env cont (get-enp-cont cont))))
           (car-exp
            (exp1)
            (value-of/k exp1 env (car-cont cont (get-enp-cont cont))))
           (cdr-exp
            (exp1)
            (value-of/k exp1 env (cdr-cont cont (get-enp-cont cont))))
           (null?-exp
            (exp1)
            (value-of/k exp1 env (null1-cont cont (get-enp-cont cont))))
           (list-exp
            (exps)
            (if (null? exps)
                (apply-cont cont (null-val))
           ...
           (diff-exp
            (exp1 exp2)
            (value-of/k exp1 env (diff1-cont exp2 env cont (get-enp-cont cont))))
           (zero?-exp
            (exp1)
            (value-of/k exp1 env (zero1-cont cont (get-enp-cont cont))))
           (if-exp
            (exp1 exp2 exp3)
            (value-of/k exp1 env (if-test-cont exp2 exp3 env cont (get-enp-cont cont))))
           (let-exp
            (vars exps body)
            (if (null? vars)
           ...
           (call-exp
            (rator rand)
            (value-of/k rator env (rator-cont rand env cont (get-enp-cont cont))))
           (begin-exp
            (exp1 exps)
            (if (null? exps)
                ...
                (value-of/k exp1
                            env
                            (begin-exp-cont (begin-exp (car exps) (cdr exps))
                                            env
                                            cont
                                            (get-enp-cont cont)))))
           (assign-exp
            ...
            (let ((ref (apply-env env var)))
              (value-of/k exp1 env (set-rhs-cont ref cont (get-enp-cont cont)))))
           (try-exp
            (exp1 var handler-exp)
            (let ((new-cont (replace-enp-cont cont var handler-exp env)))
              (value-of/k exp1 env new-cont)))
           (raise-exp
            (exp1)
            (value-of/k exp1 env (raise1-cont (get-enp-cont cont)))))))
(define replace-enp-cont
  (lambda (cont var handler-exp env)
    (cases continuation cont
           (end-cont
            ()
            (try-cont var handler-exp env (end-cont)))
           (zero1-cont
            (saved-cont enp-cont)
            (zero1-cont saved-cont (try-cont var handler-exp env cont)))
           (let-exp-cont
            (var body saved-env saved-cont enp-cont)
            (let-exp-cont var body saved-env saved-cont (try-cont var handler-exp env cont)))
           (if-test-cont
            (exp2 exp3 saved-env saved-cont enp-cont)
            (if-test-cont exp2 exp3 saved-env saved-cont (try-cont var handler-exp env cont)))
           (diff1-cont
            (exp2 saved-env saved-cont enp-cont)
            (diff1-cont exp2 saved-env saved-cont (try-cont var handler-exp env cont)))
           (diff2-cont
            (val1 saved-cont enp-cont)
            (diff2-cont val1 saved-cont (try-cont var handler-exp env cont)))
           (rator-cont
            (exps saved-env saved-cont enp-cont)
            (rator-cont saved-env saved-cont (try-cont var handler-exp env cont)))
           (rand-cont
            (rator saved-vals cont-exps saved-env saved-cont enp-cont)
            (rator-cont rator saved-vals cont-exps saved-env saved-cont
                        (try-cont var handler-exp env cont)))
           (let-cont
            (saved-vars saved-vals cont-vars cont-exps body saved-env saved-cont enp-cont)
            (let-cont saved-vars saved-vals cont-vars cont-exps body saved-env saved-cont
                      (try-cont var handler-exp env cont)))
           (cons1-cont
            (exp2 saved-env saved-cont enp-cont)
            (cons1-cont exp2 saved-env saved-cont (try-cont var handler-exp env cont)))
           (cons2-cont
            (val1 saved-cont enp-cont)
            (cons2-cont val1 saved-cont (try-cont var handler-exp env cont)))
           (car-cont
            (saved-cont enp-cont)
            (car-cont saved-cont (try-cont var handler-exp env cont)))
           (cdr-cont
            (saved-cont enp-cont)
            (cdr-cont saved-cont (try-cont var handler-exp env cont)))
           (null1-cont
            (saved-cont enp-cont)
            (null1-cont saved-cont (try-cont var handler-exp env cont)))
           (list1-cont
            (exps saved-env saved-cont enp-cont)
            (list1-cont exps saved-env saved-cont (try-cont var handler-exp env cont)))
           (list2-cont
            (first-val saved-cont enp-cont)
            (list2-cont first-val saved-cont (try-cont var handler-exp env cont)))
           (set-rhs-cont
            (ref saved-cont enp-cont)
            (set-rhs-cont ref saved-cont (try-cont var handler-exp env cont)))
           (begin-exp-cont
            (exp saved-env saved-cont enp-cont)
            (begin-exp-cont exp saved-env saved-cont (try-cont var handler-exp env cont)))
           (try-cont
