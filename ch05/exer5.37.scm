             ...
             (procedure
              ...
              (let ((var-len (length vars))
                ...
                (cond
                 ...
                 [(< var-len val-len)
                  (eopl:printf
                   "Argument number mismatch: expect ~a, got ~a.~%"
                   var-len val-len)
                  (apply-handler enp-cont (num-val val-len))]
                 [else
                  (eopl:printf
                   "Argument number mismatch: expect ~a, got ~a.~%"
                   var-len val-len)
...
(define value-of/k
  (lambda (exp env cont enp-cont)
    (cases expression exp
           ...
           (raise-exp
            (exp1)
            (value-of/k exp1 env (raise1-cont enp-cont) enp-cont)))))
...
(check-eqv?
 (run "try (proc (x, y) -(y,-(0,x)) 3 4 5) catch (var-num) var-num")
 3)
