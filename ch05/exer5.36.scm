...
;; apply-procedure/k : Proc x Listof(ExpVal) x Cont x Listof(Cont) -> Bounce
...
;; apply-cont : Cont x Listof(Cont) x ExpVal -> Bounce
(define apply-cont
  (lambda (cont enp-cont val)
    (cases continuation cont
           ...
           (try-cont
            ...
            (apply-cont cont enp-cont val))
           (raise1-cont
            (cont)
            (apply-handler cont enp-cont val)))))
;;; apply-handler : ExpVal x Cont x Listof(Cont) -> FinalANswer
  ...
  (lambda (cont enp-cont val)
    (cases continuation cont
           (try-cont
            (var handler-exp env cont)
            (if (null? enp-cont)
                (value-of/k handler-exp (extend-env var (newref val) env) cont enp-cont)
                (value-of/k handler-exp (extend-env var (newref val) env) cont (cdr enp-cont))))
           (else
...
;;; ---------------------- Evaluate expression ----------------------
;; value-of/k : Exp x Env x Cont x Listof(Cont) -> Bounce
(define value-of/k
  (lambda (exp env cont enp-cont)
    (cases expression exp
           ...
           (try-exp
            ...
            (let ((cnt (try-cont var handler-exp env cont)))
              (value-of/k exp1 env cnt (cons cnt enp-cont))))
           (raise-exp
            (exp1)
            (if (null? enp-cont)
                (report-uncaught-exception)
                (value-of/k exp1 env (raise1-cont (car enp-cont)) enp-cont))))))
