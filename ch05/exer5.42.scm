  ...
  (lambda (cont val)
    (cases continuation cont
           ...
           (throw1-cont
            ...
            (value-of/k exp2 saved-env (throw2-cont val)))
           (throw2-cont
            (saved-val)
            (apply-cont (expval->cont val) saved-val)))))
...
(define value-of/k
  (lambda (exp env cont)
    (cases expression exp
           ...
           (letcc-exp
            (var exp1)
            (value-of/k exp1 (extend-env var (newref (cont-val cont)) env) cont))
           (throw-exp
            (exp1 exp2)
            (value-of/k exp1 env (throw1-cont exp2 env cont))))))
