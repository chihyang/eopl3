...
(define receive
  (lambda (th val)
    (cases thread th
           (cont-thread
            (t i c v)
            (cont-thread t i c val))
           (proc-thread
            (t i p v c)
            (proc-thread t i p (list val) c)))))
...
;;; sent test2, yield is must in case of producer thread missing the message
(check-eqv?
 (run "let buffer = 0
       in let producer = proc (n)
              begin
               receive another-th-id in
                 letrec
                   wait4(k) = if zero?(k)
                            then send(another-th-id, n)
                            else begin
                                   print(-(k,-200));
                                   (wait4 -(k,1))
                                 end
           ...
           in let consumer = proc ()
              ...
              in let th1 = spawn(proc (d) (producer 44))
                 in begin
                     yield();
                     send(th1, 0);
                     print(300);
