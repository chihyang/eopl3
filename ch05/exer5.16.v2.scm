...
;; add1-cont : Exp x Env x Cont -> Cont
(define add1-cont
  (lambda (exp2 env cont)
    (lambda (val)
      (value-of/k exp2 env (add2-cont val cont)))))
;; add2-cont : ExpVal x Cont -> Cont
(define add2-cont
  (lambda (val1 cont)
    (lambda (val2)
      (let ((num1 (expval->num val1))
            (num2 (expval->num val2)))
        (apply-cont cont (num-val (+ num1 num2)))))))
;; mul1-cont : Exp x Env x Cont -> Cont
(define mul1-cont
  (lambda (exp2 env cont)
    (lambda (val)
      (value-of/k exp2 env (mul2-cont val cont)))))
;; mul2-cont : ExpVal x Cont -> Cont
(define mul2-cont
  (lambda (val1 cont)
    (lambda (val2)
      (let ((num1 (expval->num val1))
            (num2 (expval->num val2)))
        (apply-cont cont (num-val (* num1 num2)))))))
;; quot1-cont : Exp x Env x Cont -> Cont
(define quot1-cont
  (lambda (exp2 env cont)
    (lambda (val)
      (value-of/k exp2 env (quot2-cont val cont)))))
;; quot2-cont : ExpVal x Cont -> Cont
(define quot2-cont
  (lambda (val1 cont)
    (lambda (val2)
      (let ((num1 (expval->num val1))
            (num2 (expval->num val2)))
        (apply-cont cont (num-val (/ num1 num2)))))))
;; rand-cont : Listof(Exp) x Env x Cont -> Cont
(define rator-cont
  (lambda (rand env cont)
    (lambda (val)
      (if (null? rand)
          (apply-procedure/k (expval->proc val) '() cont)
          (value-of/k (car rand) env (rand-cont val '() (cdr rand) env cont))))))
;; rand-cont : ExpVal x Cont -> Cont
(define rand-cont
  (lambda (rator saved-vals cont-exps env cont)
    (lambda (val)
      (if (null? cont-exps)
...
;; let-cont : Listof(Id) x Listof(ExpVal) x Listof(Id) x Listof(Exp) x Exp x
;; Env x Cont -> Cont
(define let-cont
...
;; begin-exp-cont : Exp x Env x Cont -> Cont
(define begin-exp-cont
  (lambda (exp env cont)
    (lambda (val)
      (value-of/k exp env cont))))
;; set-rhs-stmt-cont : Ref x CmdCont -> CmdCont
(define set-rhs-stmt-cont
  (lambda (ref cont)
    (lambda (val)
      (begin
        (setref! ref val)
        (apply-command-cont cont)))))
;; print-stmt-cont : Exp x Env x CmdCont -> CmdCont
  ...
  (lambda (cont)
    (lambda (val)
      (begin
        ...
        (apply-command-cont cont)))))
;; if-stmt-cont : Stmt x Stmt x Env x CmdCont -> CmdCont
(define if-stmt-cont
  (lambda (stmt1 stmt2 env cont)
    (lambda (val)
      (if (expval->bool val)
          (result-of stmt1 env cont)
          (result-of stmt2 env cont)))))
;; while-stmt-cont : Exp x Stmt x Env x CmdCont -> CmdCont
(define while-stmt-cont
  (lambda (exp stmt env cont)
    (lambda (val)
      (if (expval->bool val)
          (result-of stmt env (while-command-cont exp stmt env cont))
          (apply-command-cont cont)))))
...
;; multi-command-cont : Listof(Stmt) x Env x CmdCont -> CmdCont
(define multi-command-cont
  (lambda (stmts env cont)
    (lambda ()
      (if (null? stmts)
          (apply-command-cont cont)
          (result-of (car stmts) env (multi-command-cont (cdr stmts) env cont))))))
;; while-command-cont : Exp x Stmt x Env x CmdCont -> CmdCont
(define while-command-cont
  (lambda (exp stmt env cont)
    (lambda ()
      (value-of/k exp env (while-stmt-cont exp stmt env cont)))))
...
;; result-of : Statement x Env x CmdCont -> Unspecified
(define result-of
