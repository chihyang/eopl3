...
(define apply-cont
  (lambda ()
    (when (debug-mode?)
        (eopl:printf "Current thread's remaining time slice: ~a~%" the-time-remaining))
    (if (time-expired?)
        (begin
        ...
        (begin
          ...
          (cases continuation cont
                 ...
                 (rator-cont
                  ...
                  (if (null? exps)
                      (begin
                        ...
                        pc)
                      (begin
                        (set! cont (rand-cont val '() (cdr exps) saved-env saved-cont))
                        (set! env saved-env)
                        (set! exp (car exps))
                        (value-of/k))))
                 (rand-cont
                  (rator saved-vals cont-exps saved-env saved-cont)
                  (if (null? cont-exps)
                      (begin
                        (set! cont saved-cont)
                      ...
                      (begin
                        ...
                        (value-of/k))))
                 (let-cont
                  (saved-vars saved-vals cont-vars cont-exps body saved-env saved-cont)
                  (if (null? cont-exps)
                      (let ((l-vars (reverse (cons (car cont-vars) saved-vars)))
                            (l-vals (reverse (cons val saved-vals))))
                        (set! cont saved-cont)
                        (set! env (extend-env* l-vars
                                               (map newref l-vals)
                                               saved-env))
                        (set! exp body)
                        (value-of/k))
                      (begin
                        (set! cont (let-cont (cons (car cont-vars) saved-vars)
                                             (cons val saved-vals)
                                             (cdr cont-vars)
                                             (cdr cont-exps)
                                             body
                 ...
                 (list1-cont
                  (exps saved-env saved-cont)
                  (set! cont (list2-cont val saved-cont))
                  (set! env saved-env)
                  (set! exp exps)
                  (value-of/k))
                 (list2-cont
                  (first-val saved-cont)
                  (set! cont saved-cont)
                  (set! val (pair-val first-val val))
                  (apply-cont))
                 (set-rhs-cont
                  (ref saved-cont)
                  (begin
                    ...
                    (apply-cont)))
                 (begin-exp-cont
                  (saved-exp saved-env saved-cont)
                  (set! cont saved-cont)
                  (set! env saved-env)
                  (set! exp saved-exp)
                  (value-of/k))
                 ...
                 (wait-cont
                  (saved-cont)
                  (set! mut (expval->mutex val))
                  (set! th (cont-thread the-max-time-slice saved-cont (num-val 52)))
                  (wait-for-mutex))
                 (signal-cont
                  (saved-cont)
                  (set! mut (expval->mutex val))
                  (set! th (cont-thread the-max-time-slice saved-cont (num-val 53)))
                  (signal-mutex))
                 (print-cont
                  (saved-cont)
                  (begin
           ...
           (list-exp
            ...
            (if (null? exps)
                (begin
                  ...
                  (apply-cont))
                (begin
                  (set! cont (list1-cont (list-exp (cdr exps)) env cont))
                  (set! exp (car exps))
                  (value-of/k))))
           (diff-exp
            (exp1 exp2)
           ...
           (begin-exp
            ...
            (if (null? exps)
                (begin
                  ...
                  (value-of/k))
                (begin
                  (set! cont (begin-exp-cont (begin-exp (car exps) (cdr exps)) env cont))
                  (set! exp exp1)
                  (value-of/k))))
           (assign-exp
            (var exp1)
...
(define value-of-program
  (lambda (timeslice prog debug?)
    ...
    (cases program prog
           (a-program
            (exp1)
            (set! cont (end-mainthread-cont))
            (set! env (empty-env))
            (set! proc1 'uninitialized)
            (set! val 'uninitialized)
            (set! pc #f)
            (set! exp exp1)
            (set! mut 'uninitialized)
            (set! th 'uninitialized)
            (let ((final-val (trampoline (value-of/k))))
              (expval->schemeval final-val))))))
