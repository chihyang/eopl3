...
(define the-current-store 'uninitialized)
;; get-store : () -> Sto
(define get-store
  (lambda () the-current-store))
;; initialize-store! : () -> Unspecified
;; usage : (initialize-store!) sets the store to the empty store
(define initialize-store!
  (lambda ()
    (set! the-current-store (empty-store))))
...
(define newref
  (lambda (val)
    (let ((next-ref (length the-current-store)))
      (set! the-current-store (append the-current-store (list val)))
      next-ref)))
;; deref : Ref -> ExpVal
(define deref
  (lambda (ref)
    (list-ref the-current-store ref)))
;; setref! : Ref x ExpVal -> Unspecified
;; usage : sets the-current-store to a state like the original, but with position ref
;; containing val
(define setref!
  (lambda (ref val)
    (set! the-current-store
      (letrec ((setref-inner
                ...
                (lambda (store1 ref1)
                  (cond [(null? store1)
                         (report-invalid-reference ref the-current-store)]
                        [(zero? ref1)
                         (cons val (cdr store1))]
                        [else
                         (cons (car store1)
                               (setref-inner (cdr store1) (- ref1 1)))]))))
        (setref-inner the-current-store ref)))))
    ...
    (if (time-expired?)
        (begin
          (place-on-ready-queue!
           (cont-thread the-max-time-slice the-running-thread-id cont val the-current-store))
          (run-next-thread))
        (begin
          (decrease-timer!)
          (cases continuation cont
                  ...
                  (let ((proc1 (expval->proc val))
                    ...
                    (place-on-ready-queue!
                     (proc-thread
                      ...
                      (end-subthread-cont)
                      the-current-store))
                    (apply-cont saved-cont (num-val th-id))))
                 (end-mainthread-cont
                  ()
                  (when (debug-mode?)
                 ...
                 (yield-cont
                  ...
                  (begin
                    ...
                    (if (time-expired?)
                        (place-on-ready-queue!
                         (cont-thread the-max-time-slice
                                      the-running-thread-id
                                      saved-cont
                                      val
                                      the-current-store))
              ...
              (if closed?
                  (if (empty? wait-queue)
                      ...
                      (begin
                        ...
                        (apply-thread th)))
                  (apply-thread th)))))))
;;; signal-mutex : Int x ExpVal x Cont -> FinalAnswer
(define send-to
  (lambda (id val saved-cont)
    (let ((wait-th (remove-blocking-thread! id)))
      (if (null? wait-th)
...
(define recv
  (lambda (th val)
    (cases thread th
           (cont-thread
            (t i c v sto)
            (cont-thread t i c val sto))
           (proc-thread
            (t i p v c sto)
            (proc-thread t i p (list val) c sto)))))
...
;;; the blocking threads list
(define the-blocking-threads 'unspecified)
...
;;; usage : runs next thread. If no ready threads, returns the final answer
        ...
        (begin
          ...
          (dequeue the-ready-queue
                   (lambda (first-ready-thread other-ready-thread)
                     ...
                     (set! the-current-store (thread-store first-ready-thread))
                     (apply-thread first-ready-thread)))))))
;;; remove-blocking-thread! : id -> thread
(define remove-blocking-thread!
  (lambda (id)
    (let ((found-th '()))
      (letrec ((remove!
                (lambda (lst)
...
(define place-on-blocking-list!
  (lambda (th)
    (set! the-blocking-threads (cons th the-blocking-threads))))
...
;;; producer and consumer with separate store, only send could be used
(check-eqv?
 (run "let producer = proc (n)
              letrec
                wait4(k) = if zero?(k)
                           then send(0, n)
                           else begin
...
;;; result is always 0, because every thread has different binding for x
(check-eqv?
 (run "let x = 0
      in let mut = mutex()
                           ...
                           proc (id)
                              recv x in
                               begin
                                print(dummy);
                                set x = -(x, -1);
                                if zero?(-(x, 3)) then send(0, x)
                                else send(-(id, -1), x)
                               end
          in let
              th1 = spawn((incr_x 100))
