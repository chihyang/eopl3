...
(define apply-cont
  (lambda (cont val)
    (cases continuation cont
           ...
           (zero1-cont
            (cont)
            (a-apply-cont cont (bool-val (zero? (expval->num val)))))
           (let-exp-cont
            (var body env cont)
            (a-value-of body (extend-env var val env) cont))
           (if-test-cont
            (exp2 exp3 env cont)
            (if (expval->bool val)
                (a-value-of exp2 env cont)
                (a-value-of exp3 env cont)))
           (diff1-cont
            (exp2 env cont)
            (a-value-of exp2 env (diff2-cont val cont)))
           (diff2-cont
            (val1 cont)
            (let ((num1 (expval->num val1))
                  (num2 (expval->num val)))
              (a-apply-cont cont (num-val (- num1 num2)))))
           (rator-cont
            (exps env cont)
            (if (null? exps)
           ...
           (cons1-cont
            (exp2 env cont)
            (a-value-of exp2 env (cons2-cont val cont)))
           (cons2-cont
            (val1 cont)
            (a-apply-cont cont (pair-val val1 val)))
           (car-cont
            (cont)
            (cases exp-val val
                   (pair-val
                    (first rest)
                    (a-apply-cont cont first))
           ...
           (null1-cont
            ...
            (cases exp-val val
                   (null-val
                    ()
                    (a-apply-cont cont (bool-val #t)))
                   (else (a-apply-cont cont (bool-val #f)))))
           (list1-cont
            (exps env cont)
            (a-value-of exps env (list2-cont val cont)))
           (list2-cont
            (first-val cont)
            (a-apply-cont cont (pair-val first-val val)))
           (set-rhs-cont
            (ref cont)
            (begin
              (setref! ref val)
              (a-apply-cont cont (num-val 27))))
           (begin-exp-cont
            (exp env cont)
            (a-value-of exp env cont)))))
