...
;; no-free-vars? : Exp x Exp -> Exp
(define replace-param-exp
  (lambda (body arg)
    (cases expression body
           (const-exp
            (num)
           ...
           (call-exp
            (rator rand)
            (call-exp
             (replace-param-exp rator arg)
             (replace-param-exp rand arg)))
           (else
            (report-invalid-tranlated-expression exp)))))
...
(define translation-of
  (lambda (exp senv)
    (cases expression exp
           ...
           (proc-exp
            (var body)
            (let* ((free-vars (extract-free-vars (extend-senv var (void-exp) senv) body))
                   (saved-env (car free-vars))
                   (var-pos (cdr free-vars))
                   (pos (cond [(null? var-pos) var-pos]
                              [(eqv? (car var-pos) -1) (cdr var-pos)]
                              [else var-pos]))
                   (translated-body (translation-of body (car free-vars))))
              (if (no-free-vars? translated-body 1)
                  (nameless-inline-proc-exp translated-body)
                  (nameless-proc-exp translated-body pos))))
           (call-exp
            (rator rand)
            (let ((rator-exp (translation-of rator senv))
                  (rand-exp (translation-of rand senv)))
              (cases expression rator-exp
                     (nameless-inline-proc-exp
                      (body)
                      (replace-param-exp body rand-exp))
                     (else
