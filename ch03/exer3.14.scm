...
(define value-of-if-exp
  (lambda (exp1 exp2 exp3 env)
    (let ((val (value-of-bool-exp exp1 env)))
      (if (expval->bool val)
          (value-of exp2 env)
          (value-of exp3 env)))))
...
(define value-of-bool-exp
  (lambda (exp env)
    (cases bool-exp exp
           (null-bool-exp
            (sub-exp)
            (value-of-null?-exp sub-exp env))
           (zero-bool-exp
            (sub-exp)
            (value-of-zero?-exp sub-exp env))
           (equal-bool-exp
            (exp1 exp2)
            (value-of-equal?-exp exp1 exp2 env))
           (greater-bool-exp
...
(eqv?
 ...
 5/3)
(eqv?
 (run "if null?(cdr(cons(1,emptylist))) then 1 else 2")
 1)
(eqv?
 (run "if zero?(1) then 1 else 2")
 2)
(eqv?
 (run "if equal?(let x = 1 in let y = 2 in -(x, y), -1) then 1 else 2")
 1)
(eqv?
 (run "if greater?(let x = 1 in let y = 2 in -(x, y), -1) then 1 else 2")
 2)
(eqv?
 (run "if less?(let x = 1 in let y = 2 in -(x, y), -1) then 1 else 2")
 2)
