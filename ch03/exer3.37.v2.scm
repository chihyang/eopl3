  ...
  (lambda (p-names b-vars bodies saved-env)
    (let ((vec (make-vector (length p-names))))
      (let ((new-env (extend-env p-names vec saved-env)))
        (make-proc-vec! vec 0 b-vars bodies)
        new-env))))
(define make-proc-vec!
  (lambda (vec n b-vars bodies)
    (cond ([null? b-vars] vec)
          (else
           (vector-set!
            vec n (proc-val (procedure (car b-vars) (car bodies))))
           (make-proc-vec! vec (+ n 1) (cdr b-vars) (cdr bodies))))))
(define apply-env
           ...
           (procedure
            ...
            (let ((var-len (length vars))
              ...
              (cond ((< var-len val-len)
                    ...
                    (else
                     (value-of body (extend-env vars (list->vector vals) env)))))))))
