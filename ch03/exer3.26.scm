...
(define value-of
  (lambda (exp env)
    (cases expression exp
           ...
           (proc-exp
            (var body)
            (proc-val (procedure var body (extract-free-vars env exp))))
           (call-exp
            (rator rand)
            ...
            (let ((val (value-of exp (empty-env))))
              (cases exp-val val
                     ...
                     (proc-val
                      (val)
                      val)))))))
;; extract-free-vars : Env x Exp -> Env
;;
;; usage: return an Env including all the free variables in Exp
(define extract-free-vars
  (lambda (env proc)
    (if (empty-env? env)
        env
        (if (occur-free? (cadr env) proc)
            (extend-env (cadr env) (caddr env)
                        (extract-free-vars (cadddr env) proc))
            (extract-free-vars (cadddr env) proc)))))
(define occur-free?
  (lambda (var exp)
    (cases expression exp
           ...
           (let-exp
            (let-var exp1 body)
            (and (not (eqv? var let-var))
                 (or (occur-free? var exp1)
                     (occur-free? var body))))
           (proc-exp
            (proc-var body)
            (and (not (eqv? var proc-var))
                 (occur-free? var body)))
           (call-exp
            (rator rand)
            (or (occur-free? var rator)
                (occur-free? var rand))))))
