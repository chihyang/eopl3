#lang eopl
;;; --------------------- Notes ----------------------
;; Q: define a new kind of data type or add an indicator to already defined data
;; types?
;; A: I do not know which is better, so let's try them both. This is the first
;; version of exercise 3.27
;;
;;
;; Q: print the whole parameter or just a part of it?
;; A: if it is a function value, print its name (if bound), otherwise its value
           ...
           (call-exp
            ...
            (let ((func (value-of rator env))
              ...
              (cases exp-val func
                     ...
                     (traceproc-val
                      (proc1)
                      (apply-trace-procedure proc1 arg))
                     (else
  ...
  (lambda (proc1 val)
    (cases proc proc1
           (procedure
            ...
            (display-val val)
            (display "\n")
            (let ((ret (value-of body (extend-env var val saved-env))))
              (display "< ")
              (display-val ret)
              (display "\n")
              ret)))))
