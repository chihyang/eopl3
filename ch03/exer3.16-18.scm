...
(define value-of
  (lambda (exp env)
    (cases expression exp
           ...
           (unpack-exp
            (var exp1 body)
            (value-of-unpack-exp var exp1 body env))
           (cond-exp
            (cond-list val-list)
...
(define value-of-let*-exp
  (lambda (var exp body env)
    (value-of body (extend-env-list* var exp env))))
(define value-of-unpack-exp
  (lambda (vars exp body env)
    (let ((vals (pair->list-iter (value-of exp env) '())))
...
(define report-invalid-unpack-exp
  (lambda ()
    (eopl:error
     'exp-val
     "Unpack value expression's number is not equal to identifier's")))
