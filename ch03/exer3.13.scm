...
(define value-of-zero?-exp
  (lambda (exp env)
    (num-val (if (eqv? (expval->num (value-of exp env)) 0) 1 0))))
(define value-of-equal?-exp
  (lambda (exp1 exp2 env)
    (num-val (if (eqv? (expval->num (value-of exp1 env))
                       (expval->num (value-of exp2 env)))
                 1 0))))
(define value-of-greater?-exp
  (lambda (exp1 exp2 env)
    (num-val (if (> (expval->num (value-of exp1 env))
                    (expval->num (value-of exp2 env)))
                 1 0))))
...
(define value-of-cond-exp
  (lambda (cond-list val-list env)
    (if (null? cond-list)
        ...
        (if (eqv? (expval->num (value-of (car cond-list) env)) 1)
...
(eqv?
 (run "if 2 then
         ...
         +(3, 1)")
 2)
(eqv? (run "if emptylist then 0 else 1") 1)
(eqv? (run "if list(1, 2) then 0 else 1") 0)
(eqv? (run "if cons(1, 2) then 0 else 1") 0)
(eqv? (run "if car(cons(1, 2)) then 0 else 1") 0)
(eqv? (run "if greater?(2, 2) then 0 else 1") 1)
