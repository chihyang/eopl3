...
(define method->host-name
  (lambda (m)
    (cases method m
           (a-method
            (vars body h-name s-name f-names prop final?)
            h-name))))
    ...
    (map
     (lambda (m-decl)
       (cases method-decl m-decl
              (a-method-decl
               (m-defn)
               (method-decl->method m-defn host-name super-name field-names #f))
              (a-final-method-decl
               (m-defn)
               (method-decl->method m-defn host-name super-name field-names #t))))
     m-decls)))

(define method-decl->method
  (lambda (m-defn host-name super-name field-names final?)
    (cases method-defn m-defn
           (default-method-defn
             (m-name vars body)
             (list m-name
                   (a-method vars body host-name super-name field-names (public-prop) final?)))
           (a-method-defn
            (prop m-name vars body)
            (list m-name
                  (a-method vars body host-name super-name field-names prop final?))))))

;;; merge-method-envs : MethodEnv x MethodEnv -> MethodEnv
(define merge-method-envs
  (lambda (super-m-env self-m-env)
    (check-final! super-m-env self-m-env)
    ...
    (for-each
     (lambda (m)
       (cases method (cadr m)
              (a-method
               (vars body h-name s-name f-names prop final?)
               (let ((maybe-pair (assq (car m) super-m-env)))
                 (if maybe-pair
                     (cases method (cadr maybe-pair)
                            (a-method
                             (vars body h-name s-name f-names prop final?)
                             (if final?
                                 (report-method-cannot-be-override h-name (car maybe-pair))
                                 #f)))
                     #f)))))
     self-m-env)))

(define report-method-cannot-be-override
  (lambda (c-name m-name)
    (eopl:error 'merge-method-envs "Method ~s::~s cannot be override!" c-name m-name)))
