...
(require "exer9.09.env.scm")
(require "exer9.09.class.scm")
    ...
    (cases expression exp
           ...
           (superfieldref-exp
            ...
            (let ((obj (expval->obj (deref (apply-env env '%self))))
                  (super-class (lookup-class (apply-env env '%super))))
              (let ((pos (index-of field-name (reverse (class->field-names super-class)))))
                (if pos
                    (let ((ref (field->ref
                                (list-ref (object->fields obj)
                                          (- (length (class->field-names super-class))
                                             pos
                                             1)))))
                      (deref ref))
                    (report-filed-not-found obj field-name)))))
