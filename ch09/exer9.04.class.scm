...
(define find-method
  (lambda (c-name name)
    (let ((clss (lookup-class c-name)))
      (find-method-from-class clss name))))

;;; find-method-from-class : Class x Sym -> Method
(define find-method-from-class
  (lambda (clss name)
    (let ((m-env (class->method-env clss)))
      (let ((maybe-pair (assq name m-env)))
        (if maybe-pair
            (cadr maybe-pair)
            (report-method-not-found clss name))))))
