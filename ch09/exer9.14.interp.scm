...
(define apply-method
  (lambda (m self args)
    (cases method m
           (a-method
            (vars body host-name super-name field-names)
            (value-of
             body
             (extend-env* vars (map newref args)
                          (extend-env-with-self-and-super
                           (newref (cons (obj-val self) host-name)) super-name
  ...
  (lambda (exp env)
    (cases expression exp
           ...
           (method-call-exp
            ...
            (let ((args (value-of-exps rands env)))
              (cases expression obj-exp
                     (self-exp
                      ()
                      (let ((obj-with-host-name (deref (apply-env env '%self))))
                        (let ((obj (expval->obj (car obj-with-host-name)))
                              (host-name (cdr obj-with-host-name)))
                          (apply-method
                           (find-method host-name method-name)
                           obj
                           args))))
                     (else
           ...
           (super-call-exp
            (method-name rands)
            (let ((args (value-of-exps rands env))
                  (obj (expval->obj (car (deref (apply-env env '%self))))))
