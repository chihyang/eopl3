...
(define initialize-class-decl!
  (lambda (c-decl)
    (cases class-decl c-decl
           (a-class-decl
            (c-name s-name host-f-names m-decls)
            (let ((super-fields (class->field-names (lookup-class s-name))))
              (let ((f-names (append-field-names super-fields host-f-names)))
                (add-to-class-env!
                 c-name
...
;;; Listof(FieldName) x Int -> MethodEnv
(define method-decls->method-env
  (lambda (m-decls super-name field-names super-fields-number)
    (map
     (lambda (m-decl)
       (cases method-decl m-decl
              (a-method-decl
               (m-name vars body)
               (list m-name
                     (a-method vars body super-name field-names super-fields-number)))))
     m-decls)))
