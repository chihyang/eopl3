...
(define apply-method
  (lambda (m self args)
    (cases method m
           (a-method
            (vars body super-name field-names super-fields-number)
            (value-of
             body
             (extend-env* vars (map newref args)
                          (extend-env-with-self-and-super
                           ...
                           (extend-env* field-names
                                        (sublist (object->fields self) super-fields-number)
                                        (empty-env)))))))))

;;; sublist : List x Int -> List
(define sublist
  (lambda (lst pos)
    (if (zero? pos)
        lst
        (sublist (cdr lst) (- pos 1)))))
