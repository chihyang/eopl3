           ...
           (extend-env-with-self-and-super
            ...
            (cond [(eqv? search-var '%self)
                  ...
                  [else
                   (apply-env saved-env search-var)]))
           (extend-env-with-field
            (f-names fs saved-env)
            (let ((pos (index-of search-var f-names)))
              (if pos
                  (list-ref fs pos)
                  (apply-env saved-env search-var)))))))

;;; index-of : Sym x Listof(SchemeVal) -> Int | Bool
(define index-of
  (lambda (sym lst)
    (let loop ((lst lst)
               (sym sym)
...
;;; object->fields : Object -> Listof(Field)
...
;;; object-search-field : Object x Name -> Field | Bool
(define object-search-field
  (lambda (obj name)
    (cases object obj
           (an-object
            (c-name fields)
            (search-fields fields name)))))

;;; search-fields : Listof(Field) x Name -> Field | Bool
(define search-fields
  (lambda (fields name)
    (if (null? fields)
        #f
        (if (eqv? (field->name (car fields)) name)
            (car fields)
            (search-fields (cdr fields) name)))))
    ...
    (cases exp-val v
           ...
           (obj-val
            ...
            (cases object val
                   (an-object
                    (c-name fields)
                    (list c-name (map (lambda (f) (expval->schemeval (deref (field->ref f)))) fields))))))))
