...
(define object-field?
  (lambda (field)
    (and (pair? field)
         (identifier? (car field))
         (reference? (cadr field))
         (null? (cddr field)))))
...
(define object->fields
  (lambda (obj)
    (cases object obj
           (an-object
            (c-name fields)
            (map (lambda (f) (cadr f)) fields)))))

;;; object-search-field : Object x Name -> Ref | Bool
(define object-search-field
  (lambda (obj name)
    (cases object obj
           (an-object
            (c-name fields)
            (let ((pair (assq name fields)))
              (if pair (cadr pair) #f))))))
    ...
    (cases exp-val v
           ...
           (obj-val
            ...
            (cases object val
                   (an-object
                    (c-name fields)
                    (list c-name (map (lambda (f) (expval->schemeval (deref (cadr f)))) fields))))))))
