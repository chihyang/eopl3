...
(define expand-extend-tenv
  (lambda (var ty tenv)
    (let ((ety (expand-type ty tenv)))
      (extend-tenv var ety tenv))))

(define expand-extend-tenv-with-type
  (lambda (var ty tenv)
    (let ((ety (expand-type ty tenv)))
      (extend-tenv-with-type var ety tenv))))
  ...
  (lambda (defns env)
    (if (null? defns)
        ...
        (cases definition (car defns)
               ...
               (type-defn
                (name ty)
                (let ((new-env (expand-extend-tenv-with-type
                                name ty env)))
    ...
    (cases expression exp
           ...
           (let-exp
            ...
            (let ((b-type (type-of-exp b-exp tenv)))
              (type-of-exp let-body (expand-extend-tenv b-var b-type tenv))))
           (letrec-exp
            (result-type p-name b-var arg-type b-body letrec-body)
            (let ((letrec-tenv (expand-extend-tenv
                                p-name
                                (proc-type arg-type result-type)
                                tenv)))
              (let ((checked-type (type-of-exp
                                   b-body
                                   (expand-extend-tenv b-var arg-type letrec-tenv))))
