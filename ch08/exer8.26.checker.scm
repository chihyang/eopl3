...
(define interface-of
  (lambda (m-body env)
    (cases module-body m-body
           ...
           (app-module-body
            (rator rand)
            (let ((rator-iface (interface-of rator env)))
              ;; how to replace?
              (let ((rand-iface (interface-of rand env)))
                (cases interface rator-iface
                       (proc-iface
                        (var var-iface result-iface)
                        (if (<:-iface rand-iface var-iface env)
                            (if (var-module-body? rand)
                                (rename-in-iface result-iface
                                                 var
                                                 (module-body->var rand))
                                result-iface)
                            (report-module-doesnt-satisfy-iface
