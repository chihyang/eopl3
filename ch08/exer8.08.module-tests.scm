 ...
 body
  ...
  -(from m take u, from m take v)"
    ...
    (modules-check-duplicate-defn
     "module m1 interface [u : int] body [u = 44]
      module m1 interface [v : int] body [v = -(from m1 take u,11)]
         -(from m1 take u, from m2 take v)"
     error)
    ...
    (letrec-in-module-defn
     "module m interface
               [ odd  : (int -> int)
                 even : (int -> int) ]
             body
               letrec int local-odd(x : int)  = if zero?(x) then 0 else (local-even -(x,1))
    ...
    (nested-module-defn-1
     "module m interface
               [ x : int
                 y : int ]
             body
               module m1 interface [ x : int ] body [ x = 2 ]
               let y1 = 4
               in [ x = from m1 take x
                    y = y1 ]
     -(from m take x, from m take y)"
    ...
    (moduel-decl-defn-different-order
     "module m
      interface [ x : int y : bool ]
      body [ y = zero?(0) x = 3 ]
      from m take y"
     bool #t)))
