...
;;; extend-env-with-module* : Listof(Sym) x Listof(Module) x Env -> Env
(define extend-env-with-module*
  (lambda (m-names m-vals saved-env)
    (check-duplicate-identifier! m-names)
    (check-var-val-number! m-names m-vals)
    (let loop ((names m-names)
               (vals m-vals)
               (env saved-env))
      (if (null? vals)
          env
...
;;; check-var-val-number! : Listof(Sym) x Listof(Module) -> Bool | Unspecified
