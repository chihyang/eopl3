...
;;; extend-tenv-with-module* : Listof(Sym) x Listof(Interface) x TypeEnv -> TypeEnv
(define extend-tenv-with-module*
  (lambda (names ifaces saved-env)
    (check-duplicate-identifier! names)
    (check-name-iface-number! names ifaces)
    (let loop ((names names)
      ...
      (if (null? names)
          ...
          (loop (cdr names)
                (cdr ifaces)
                (extend-tenv-with-module (car names) (car ifaces) tenv))))))
...
;;; check-name-iface-number! : Listof(Sym) x Listof(Module) -> Bool | Unspecified
(define check-name-iface-number!
  (lambda (vars ifaces)
    (let ((var-len (length vars))
          (val-len (length ifaces)))
      (cond [(< var-len val-len)
             (report-iface-number-mismatch 'more vars ifaces)]
            [(> var-len val-len)
             (report-iface-number-mismatch 'less vars ifaces)]
            [else #t]))))
  ...
  (lambda (m-name var-name env)
    (let ((iface (lookup-module-name-in-tenv env m-name)))
      (cases interface iface
             ...
             (proc-iface
              (m-names bound-ifaces returned-iface)
...
(define report-iface-number-mismatch
  (lambda (symp names ifaces)
    (eopl:error 'extend-tenv-with-module*
...
(define interface-of
  (lambda (m-body env)
    (cases module-body m-body
           ...
           (proc-module-body
            (m-names var-ifaces m-body)
            (let ((expand-var-ifaces
                   (map (lambda (n i) (expand-iface n i env))
                        m-names
                        var-ifaces)))
              (let ((new-env (extend-tenv-with-module* m-names expand-var-ifaces env)))
                (let ((result-iface (interface-of m-body new-env)))
                  ;; it is worth noting that proc-iface does not have a similar
                  ;; closure to procedure, just like a proc-type, because no
                  ;; matter in what environment, the type of a
                  ;; procedure/procedure module should be the same, because this
                  ;; is a CHECKED language, not a INFER language
                  (proc-iface m-names var-ifaces result-iface)))))
           (var-module-body
            (m-name)
           ...
           (app-module-body
            ...
            (let ((rator-iface (lookup-module-name-in-tenv env rator)))
              (cases interface rator-iface
                     (proc-iface
                      (vars var-ifaces result-iface)
                      (let ((rand-ifaces
                             (map (lambda (r) (lookup-module-name-in-tenv env r)) rands)))
                        (if (<:-ifaces rand-ifaces var-ifaces env)
                            ;; replace var with rand in result-iface and return
                            (renames-in-iface result-iface vars rands)
                            (report-module-doesnt-satisfy-iface
                             rands var-ifaces rand-ifaces))))
                     (else
...
;;; <:-ifaces : Listof(Interface) x Listof(Interface) x TypeEnv -> Bool
(define <:-ifaces
  (lambda (rand-ifaces var-ifaces env)
    (if (eqv? (length rand-ifaces) (length var-ifaces))
        (let loop ((rand-ifaces rand-ifaces)
                   (var-ifaces var-ifaces))
          (if (null? rand-ifaces)
              #t
              (if (<:-iface (car rand-ifaces) (car var-ifaces) env)
                  (loop (cdr rand-ifaces) (cdr var-ifaces))
                  #f)))
        #f)))
           ...
           (proc-iface
            ...
            (cases interface iface2
                   ...
                   (proc-iface
                    ...
                    (if (eq? (length param-iface1) (length param-iface2))
                        (let ((new-names (map fresh-module-name param-name1)))
                          (let ((result-iface1
                                 (renames-in-iface
                                  result-iface1 param-name1 new-names))
                                (result-iface2
                                 (renames-in-iface
                                  result-iface2 param-name2 new-names)))
                            (and (<:-ifaces param-iface2 param-iface1 tenv)
                                 (<:-iface result-iface1 result-iface2
                                           (extend-tenv-with-module*
                                            new-names
                                            (map (lambda (n i) (expand-iface n i tenv))
                                                 new-names param-iface1)
                                            tenv)))))
                        #f)))))))

;;; renames-in-iface : Interface x Listof(Sym) x Listof(Sym) -> Listof(Iface)
(define renames-in-iface
  (lambda (iface old-names new-names)
    (if (null? old-names)
        iface
        (renames-in-iface (rename-in-iface iface (car old-names) (car new-names))
                          (cdr old-names) (cdr new-names)))))
           ...
           (proc-iface
            ...
            (if (eq? param-name1 old-name)
                (proc-iface param-name1
                            (map (lambda (i) (rename-in-iface i old-name new-name))
                                 param-iface1)
                            result-iface1)
                (proc-iface param-name1
                            (map (lambda (i) (rename-in-iface i old-name new-name))
...
(define report-argument-number-mismatch
  (lambda (symp vars types)
    (eopl:error 'interface-of
                "Argument number is ~s than parameter number")))
