...
(define <:-decls
  (lambda (decls1 decls2 tenv)
    (if (null? decls2)
        #t
        (if (in-decls? (car decls2) decls1 tenv)
            (<:-decls decls1 (cdr decls2) tenv)
            #f))))

(define in-decls?
  (lambda (a-decl decls tenv)
    (cond [(null? decls)
           #f]
          [(equal? (decl->name a-decl) (decl->name (car decls)))
           (<:-decl (car decls) a-decl tenv)]
          [else
           (in-decls? a-decl
                      (cdr decls)
                      (extend-tenv-with-decl (car decls) tenv))])))
