...
(define cps-unparse-simple-exp
  (lambda (exp)
    (cases simple-exp exp
           ...
           (cps-car-exp
            (exp1)
            `(car ,(cps-unparse-simple-exp exp1)))
           (cps-cdr-exp
            (exp1)
            `(car ,(cps-unparse-simple-exp exp1)))
           (cps-null?-exp
            (exp1)
            `(null? ,(cps-unparse-simple-exp exp1)))
           (cps-cons-exp
            (exp1 exp2)
            `(cons ,(cps-unparse-simple-exp exp1) ,(cps-unparse-simple-exp exp2)))
           (cps-list-exp
            (exps)
            (append 'list (map cps-unparse-simple-exp exps))))))
