...
in -((g 11),(g 22))"
 ...
 in -((g 11),(g 22))"
    ...
    (even-odd-via-set-1 "
let x = newref(0)
in letrec even(f) = proc (d) if zero?(deref(x))
                   then 1
                   else let d = setref(x, -(deref(x),1))
                        in (f d)
in letrec odd(d)  = if zero?(deref(x))
                   then 0
                   else let d = setref(x, -(deref(x),1))
                        in ((even odd) d)
   in let d = setref(x,13) in (odd d)" 1)
...
in fact"
 ...
 in -((g 11),(g 22))"
    ...
    ;; allowed in interpreter but not allowed in checker
    (even-odd-via-set-1 "
let x = newref(0)
in letrec even(f) = proc (d)
                   if zero?(deref(x))
                   then 1
                   else let d = setref(x, -(deref(x),1))
                        in (f d)
in letrec odd(d)  = if zero?(deref(x))
                   then 0
                        ...
                        in ((even odd) d)
   in let d = setref(x,13) in (odd -100)" error)

    (even-odd-via-set-2 "
let x = newref(0)
in letrec even(odd) = proc (d) if zero?(deref(x))
                   then 1
                   else begin setref(x, -(deref(x),1)); 23 end
   in letrec odd(d)  = if zero?(deref(x))
                   then 0
                   else begin setref(x, -(deref(x),1)); 23 end
   in let d = setref(x,13) in (odd d)" int)
...
in begin
    ...
    ;; this case is supposed to fail in basic case
    (let-polymorphic-type-with-newref
