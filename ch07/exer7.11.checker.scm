...
(define type-of-exp
  (lambda (exp tenv)
    (cases expression exp
           ...
           (proc-exp
            (vars tys body)
            (let ((result-type (type-of-exp body (extend-tenv* vars (map newref tys) tenv))))
              (proc-type tys result-type)))
           (let-exp
            (b-vars b-exps let-body)
            (let ((b-types (type-of-exps b-exps tenv)))
              (type-of-exp let-body (extend-tenv* b-vars (map newref b-types) tenv))))
           (letrec-exp
            (result-types p-names b-vars arg-types b-bodies letrec-body)
            (let ((letrec-tenv
                   (extend-tenv*
                    p-names
                    (map newref (map proc-type arg-types result-types))
                    tenv)))
              (let ((checked-types
                     (map (lambda (b-body b-var arg-type)
                            (type-of-exp
                             b-body
                             (extend-tenv* b-var (map newref arg-type) letrec-tenv)))
                          b-bodies
                          b-vars
                          arg-types)))
           ...
           (assign-exp
            (var exp1)
            (let ((new-type (type-of-exp exp1 tenv)))
              (replace-type-in-tenv! tenv var new-type)
              (int-type)))
           (newpair-exp
            (exp1 exp2)
            (let ((ty1 (type-of-exp exp1 tenv))
                  (ty2 (type-of-exp exp2 tenv)))
              (mutpair-type ty1 ty2)))
           (left-exp
            (exp1)
            (let ((ty1 (type-of-exp exp1 tenv)))
              (cases type ty1
                     (mutpair-type (tyl tyr) tyl)
                     (else
                      (report-not-a-mutpair-type ty1 exp1)))))
           (right-exp
            (exp1)
            (let ((ty1 (type-of-exp exp1 tenv)))
              (cases type ty1
                     (mutpair-type (tyl tyr) tyr)
                     (else
                      (report-not-a-mutpair-type ty1 exp1)))))
           (setleft-exp
            (exp1 exp2)
...
(define report-not-a-mutpair-type
  (lambda (ty1 exp)
    (eopl:error 'type-of-exp
                "Expect a mutpair, actual ~a: ~a"
                (type-to-external-form ty1)
                exp)))
