...
in -((g 11),(g 22))"
 ...
 in -((g 11),(g 22))"
    ...
    (even-odd-via-set-1 "
let x = newref(0)
in letrec int even(d : int) = if zero?(deref(x))
                   then 1
                   else let d = setref(x, -(deref(x),1))
                        in (odd d)
          int odd(d : int)  = if zero?(deref(x))
                   then 0
                   else let d = setref(x, -(deref(x),1))
                        in (even d)
...
in letrec int even(d : int) = if zero?(deref(x))
   ...
   in let d = setref(x,13) in (odd -100)" error)
    ...
    (even-odd-via-set-2 "
let x = newref(0)
in letrec int even() = if zero?(deref(x))
                   then 1
                   else begin setref(x, -(deref(x),1)); 23 end
          int odd()  = if zero?(deref(x))
                   then 0
                   else begin setref(x, -(deref(x),1)); 23 end
   in let d = setref(x,13) in (odd)" int)
