...
in let f = proc (z : int) let zz = -(z,x) in zz
   in -((f 66), (f 55))"
    ...
    (mutpair-test-1 "let glo = pair(11,22)
        in let f = proc (loc : mutpair int * int)
    ...
    (mutpair-test-2 "let x = pair(11,22)
                     in begin setright(x, -(right(x), 1));
                              setleft(x, zero?(left(x)));
                              x
                        end"
                    (mcons #f 21))
    ))
...
in -((g 11),(g 22))"
    ...
    (simple-store-test-1 "let x = 17 in x" int)
...
in letrec int even(d : int) = if zero?(x)
          ...
          int odd(d : int)  = if zero?(x)
                   then 0
                        ...
                        in (even d)
   in let d = set x = 13 in (odd -100)" int)
...
in let f = proc (z : int) let zz = -(z,x) in zz
   in -((f 66), (f 55))"
    ...
    (set-change-type "let x = 3 in begin set x = zero?(x); x end"
                     bool)
