           ...
           (newref-exp
            ...
            (let ((tvar (fresh-tvar-type)))
              (cases answer (type-of-exp exp1 tenv subst)
                     (an-answer
                      (t1 subst)
                      (let ((subst (unifier tvar
                                            (ref-type t1)
                                            subst
                                            exp)))
                        (an-answer (ref-type t1) subst))))))
           (deref-exp
            (exp1)
            (let ((tvar (fresh-tvar-type)))
              (cases answer (type-of-exp exp1 tenv subst)
            ...
            (cases answer (type-of-exp exp1 tenv subst)
                   (an-answer
                    ...
                    (cases answer (type-of-exp exp2 tenv subst)
                           (an-answer
                            (t2 subst)
                            (let ((subst (unifier t1 (ref-type t2) subst exp)))
                              (an-answer (void-type) subst)))))))
           (begin-exp
            (exp1 exps)
            (if (null? exps)
                (type-of-exp exp1 tenv subst)
                (cases answer (type-of-exp exp1 tenv subst)
                       (an-answer
                        (t1 subst)
                        (type-of-exp (begin-exp (car exps) (cdr exps)) tenv subst))))))))
