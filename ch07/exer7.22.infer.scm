...
(define extend-subst!
  (lambda (tvar ty)
    (set! the-subst
      (letrec ((extend-subst-inner
                (lambda (subst1 ref1)
                  (cond [(null? subst1)
                         (report-invalid-tvar tvar)]
                        [(zero? ref1)
                         (cons ty (cdr subst1))]
                        [else
                         (cons (car subst1)
                               (extend-subst-inner (cdr subst1) (- ref1 1)))]))))
        (cases type tvar
               (tvar-type
                (sn)
                (extend-subst-inner the-subst sn))
               (else
                (report-invalid-tvar tvar)))))))
...
(define fresh-tvar-type
  (lambda ()
    (let ((sn (length (get-subst))))
      (set! the-subst (append the-subst (list #f)))
      (tvar-type sn))))
...
(define subst-of
  (lambda (tvar)
    (cases type tvar
           (tvar-type
            (sn)
            (if (< sn (length the-subst))
                (list-ref the-subst sn)
                #f))
           (else
            (report-invalid-tvar tvar)))))
...
(define report-invalid-tvar
  (lambda (type)
    (eopl:error 'subst-of "Not a tvar: ~a~%" type)))
